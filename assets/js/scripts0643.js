...
function getPrice() {
  $.getJSON(
    "https://api.pancakeswap.info/api/v2/tokens/0x2c8368f8f474ed9af49b87eac77061beb986c2f1",
    function (data) {
      $(".currentprice").html("$" + toFixed(data.data.price, 4));
    }
  );

  $.getJSON(
    "https://api.allorigins.win/get?url=https://api.kanga.exchange/api/v2/market/ticker",
    function (data) {
      var contents = $.parseJSON(data.contents);
      for (t in contents) {
        if (t == "LEON-USDT") {
          $(".currentpriceleon").html(
            "$" + toFixed(contents[t]["last_price"], 4)
          );
        }
      }
    }
  );
  $.getJSON("https://token.leonicornswap.com/history", function (data) {
    $(".currentburned").html(
      "Burned - " +
        number_format(
          data.totalburn + data.deadburn + data.accburn,
          0,
          ".",
          ","
        ) +
    ...
    $(".currentcirculation").html(
      number_format(data.circulating, 0, ".", ",") + " Tokens"
    );
    $(".currentsupply").html(
      number_format(
        210000000 - (data.totalburn + data.deadburn + data.accburn),
        0,
        ".",
        ","
      ) + " Tokens"
  ...
  NioApp.Plugins.countdown = function () {
    ...
    if ($count.exists()) {
      $count.each(function () {
        ...
        var _day_text = $self.data("day-text")
          ? $self.data("day-text")
          : "Days";
  ...
  NioApp.Plugins.carousel = function () {
    ...
    if ($carousel.exists()) {
      $carousel.each(function () {
        ...
        var cim_t_l = $self.data("items-tab-l")
          ? $self.data("items-tab-l")
          : cim > 3
          ? cim - 1
          : cim;
        var cim_t_p = $self.data("items-tab-p")
          ? $self.data("items-tab-p")
          : cim_t_l > 2
          ? cim_t_l - 1
          : cim_t_l;
        var cim_m = $self.data("items-mobile")
          ? $self.data("items-mobile")
          : cim_t_p > 1
          ? cim_t_p - 1
          : cim_t_p;
        var cim_xm = $self.data("items-mobile-s")
          ? $self.data("items-mobile-s")
          : cim_m;
        var c_timeout = $self.data("timeout") ? $self.data("timeout") : 6000;
        var c_auto = $self.data("auto") ? $self.data("auto") : false,
          c_auto_speed = $self.data("auto-speed")
            ? $self.data("auto-speed")
            : 600,
          c_loop = $self.data("loop") ? $self.data("loop") : false,
          c_dots = $self.data("dots") ? $self.data("dots") : false,
          c_custdots = $self.data("custom-dots")
            ? "." + $self.data("custom-dots")
            : false,
          ...
          c_mgn_t_l = $self.data("margin-tab-l")
            ? $self.data("margin-tab-l")
            : c_mgn,
          c_mgn_t_p = $self.data("margin-tab-p")
            ? $self.data("margin-tab-p")
            : c_mgn_t_l,
          c_mgn_mob = $self.data("margin-mob")
            ? $self.data("margin-mob")
            : c_mgn_t_p,
          c_animate_out = $self.data("animate-out")
            ? $self.data("animate-out")
            : false,
          c_animate_in = $self.data("animate-in")
            ? $self.data("animate-in")
            : false;
    ...
    function fixing_blank(_this) {
      ...
      if (!_self.hasClass("blank-added") && $win.width() > 575) {
        ...
        !_self.hasClass("blank-removed") &&
        $win.width() < 576 &&
        _self.hasClass("blank-added")
      ) {
  ...
  NioApp.Plugins.flexslider = function () {
    ...
    if ($FlexSlioder.exists()) {
      $FlexSlioder.each(function () {
        var $self = $(this),
          f_SlideShowSpeed = $self.data("slide-speed")
            ? $self.data("slide-speed")
            : 3000,
          f_SlideShow = $self.data("slide-show")
            ? $self.data("slide-show")
            : false,
          f_Anim_Loop = $self.data("anim-loop")
            ? $self.data("anim-loop")
            : false,
          f_Anim_Speed = $self.data("anim-speed")
            ? $self.data("anim-speed")
            : 600,
  ...
  NioApp.Plugins.select2 = function () {
    ...
    if ($select.exists()) {
      $select.each(function () {
        var $self = $(this),
          _theme = $self.data("select2-theme")
            ? $self.data("select2-theme")
            : "bordered",
          _place = $self.data("select2-placehold")
            ? $self.data("select2-placehold")
            : "Select an option";
        $self.select2({
          placeholder: _place,
    ...
    if ($particles_bg.exists()) {
      $particles_bg.each(function () {
        var $self = $(this),
          ...
          _pt_base_op = $self.data("pt-base-op")
            ? parseFloat($self.data("pt-base-op"))
            : 0.1,
          _pt_shape = $self.data("pt-shape") ? $self.data("pt-shape") : "#fff",
          _pt_shape_op = $self.data("pt-shape-op")
            ? parseFloat($self.data("pt-shape-op"))
            : 0.1,
          _pt_line = $self.data("pt-line") ? $self.data("pt-line") : "#fff",
          _pt_line_op = $self.data("pt-line-op")
            ? parseFloat($self.data("pt-line-op"))
            : 0.2;
  ...
  NioApp.Plugins.videoBG = function () {
    ...
    if ($videobg.exists()) {
      $videobg.each(function () {
        ...
        if (
          $this.hasClass("bg-video-youtube") &&
          !$this.hasClass("cover-enabled")
        ) {
          $this.YTPlayer({
...
function copyToClickBoard() {
  var content = "0x5e4efb364071c64ee3641fe1e68cb5d2d5558709";
